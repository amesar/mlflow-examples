...
def show_mlflow_uris(msg):
    ...
    print("  mlflowClient.tracking_uri:", mlflow_client.tracking_uri)
    print("  mlflowClient.registry_uri:", mlflow_client._registry_uri)
...
_model_name_path = "/tmp/Mini_MLOps_Pipeline.txt"
print("_model_name_path:", _model_name_path)
_model_name = None

def _write_model_name():
    with open(_model_name_path, "w") as f:
        f.write(_model_name)

def _read_model_name():
    if os.path.exists(_model_name_path):
        with open(_model_name_path, "r") as f:
            return f.read()
    else:
        raise RuntimeError(f"ERROR: '{_model_name_path}' missing. Run notebook 02_Register_Model first")

_model_name = _read_model_name()
